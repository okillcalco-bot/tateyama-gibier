...
> **【2026-08-02 実施済み】** 本章の内容を実装した（PR: 個体トレース）。
> **DBは一切変更していない**（マイグレーションなし・ビューも作らず、
> アプリからの読み取りクエリとドメイン層の集約だけで実現）。
> 当初案では 0030 でビューを作る予定だったが、0029 が未適用で採番が不安定なこと、
> および「館山ジビエのDBの軸は変えない」という方針を最大限に守るため、
> **DB変更ゼロの構成に変更した**。
> 実装: `src/domain/gibier/individual-trace.ts` / `/gibier/individuals` /
> `/gibier/individuals/[labelId]` / `tests/domain/individual-trace.test.ts`（8件）
...
**実際に変更したファイル（2026-08-02）**: マイグレーションは**作らなかった**。
以下の当初案から「0030」の行を削除し、他はそのまま実施している。

| 種別 | パス | 変更 |
|---|---|---|
| ~~新規~~ | ~~`0030_individual_trace_views.sql`~~ | **不要と判断**（DB変更ゼロで実現） |
