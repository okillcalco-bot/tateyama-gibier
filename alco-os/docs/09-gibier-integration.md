...
### 搬入連絡のスタッフグループ通知（0028）

捕獲者向けLINE公式アカウント（@889alcvb）をスタッフのLINEグループに招待し、
「搬入連絡」が届いたらそのグループへ Push で知らせる。

| 手順 | 動き |
|---|---|
| グループに招待 | `join` イベントで `line_staff_groups` に `pending` で記録し、案内文を返す |
| グループで「登録」 | `active` + `notify_delivery=true`。「このグループに搬入連絡を通知します」と返す |
| グループで「解除」 | `disabled`。通知を止める |
| グループから退出 | `leave` イベントで `left`。職員画面からは再開できない（再招待が必要） |
| 職員画面 `/line` | 通知のON/OFF・解除・グループ名の編集（承認権限が必要） |

**守秘義務（最小限の通知）**
- 流すのは「誰から・いつ・本日の受入可否」まで。
  **買取額・口座・捕獲場所の座標・写真・LINEユーザーIDは流さない**
  （`buildDeliveryNoticeMessage()` に集約し、テストで固定）
- 未照合の相手は名前を出さず「お名前の確認まちの方」と表示する

**誤爆防止**
- グループ・複数人トークのイベントは**業務処理しない**。
  `source.groupId` / `source.roomId` があれば webhook 側で分岐し、
  捕獲者の紐付け・捕獲報告・AI分類には一切入れない
- 反応するのは招待（join）と「登録」「解除」の2語だけ。
  それ以外のグループ発言には返信もしない

**LINE Manager 側の設定**: 「グループ・複数人トークへの参加を許可」を ON にする必要がある。
