...
**試行制限と残存リスク（正直な整理）**:
- スタッフキーの誤入力は10回/5分、回復コードは5回/15分でロック。試行は `auth_attempts` に記録
- 照合はハッシュ同士の比較（bcrypt / sha256）のため、比較の時間差からキーは推定できない
- **既知の限界**: `admin_*` RPC 経由の誤キーは例外でトランザクションごと巻き戻るため
  失敗ログが残らず、その経路の試行制限は効かない（回復コード照合は false 返却方式にして
  ログが残るよう修正済み）。この経路の総当たりは、キー長（回転時に16文字以上を強制・
  サーバ生成は128bit相当）で計算量的に不可能な水準に抑える
...
advisory lock（電話別＋全体）で直列化し、同時リクエストでも突破されない。上限到達は
