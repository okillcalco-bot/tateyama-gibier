...
### 🔄 スタッフキーの適用範囲を縮小（2026-08-12 オーナー判断）

現場運用でキー入力の手間が大きすぎるため、**スタッフキーを求めるのはマスタデータ
（顧客台帳 customers）の操作だけ**に縮小した（`20260812_orders_rls_relax.sql`）。

- orders / order_items: anon の SELECT / INSERT / UPDATE を再び開放（DELETEは開放しない）。
  出荷一覧・BASE取込・直接出荷・新着注文通知はキーなしで動く
- customers: 変更なし（キーのヘッダ必須のまま。718件の顧客マスタは引き続き遮断）
- 直接出荷の顧客紐付けは `staff_lookup_customer_id(名前)`（SECURITY DEFINER・
  名前が一意のときだけidを返す。住所・電話は返さない）で解決
- **受け入れたリスク**: anonキー保有者が注文データ（配送先の氏名・住所・電話を含む）を
  読み書きできる状態に戻る。戻す場合は `rollback/20260812_orders_rls_relax_rollback.sql`
- **キーの共有リンク**: 受発注管理→顧客管理→「📲 共有リンクをコピー」で
  `index.html#skey=…` 形式のURLを発行。開くだけで端末にキーが入る（手入力不要）。
  ハッシュはサーバーへ送信されず、開いた瞬間にURLから消える。リンク自体がキーなので
  個別トーク限定で送り、漏れた疑いがあれば「変更する」で全端末失効
- 管理者の4桁ログイン(2468)直後の自動キー確認は廃止（顧客管理を開いたときだけ確認）
...
### 9-8. お客様向け「かんたんログインリンク」（2026-08-11 追加）
`order-admin.html` の「ご案内」機能が案内文の宛先を旧 `order-portal.html`（注文停止中）に
していた不具合を修正し、`order.html` を指すようにした。あわせて、ID・パスワードの入力を省ける
本人専用リンクを追加した。

- 仕組み: `admin_issue_customer_link()`（スタッフキー必須）が `portal_login_v2()` と同じ
  `portal_sessions` トークンをパスワード入力なしで発行する。`order.html` は起動時に
  URLのハッシュ（`#t=トークン`）を読み取り、通常のログインセッションと同じ扱いで使う
  （検証側の `portal_session_customer()` 等は無改修）
- 発行対象は `portal_enabled=true` の顧客のみ（試験運用は数社に限る、の合意を維持）。
  `order-admin.html` の顧客編集フォームに「注文サイト利用を有効にする」チェックボックスを追加し、
  未有効の相手にリンクを発行しようとすると確認のうえ有効化できるようにした
- 有効期限は既定90日（1〜365日で発行者指定可）。再発行すると前回のリンクは失効する
  （パスワード再発行が旧パスワードを失効させるのと同じ挙動に揃えた）
- URLのハッシュ（`#`以降）はサーバに送信されないため、リンクプレビュー取得やアクセスログに
  トークンが残らない。ただしリンク自体はパスワードと同じ機密情報であり、渡った相手が
  ログインできる点は変わらない（LINEの個別トークなど、私信の経路で送ることを前提とする）
