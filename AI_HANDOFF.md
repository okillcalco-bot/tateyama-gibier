...
| `/gibier/individuals` `/gibier/individuals/[labelId]` | **個体トレース**（読み取り専用）。label_id を軸に 捕獲報告→個体→部位在庫→受注→買取 を1画面に集約。個体↔在庫の接続状況（孤児在庫）を可視化するが**直さない**。既存テーブルの構造・データは一切変更しない。座標は geo-masking 経由（owner/manager のみ原座標） |
