-- 捕獲者名を「この肉の物語」（公開ページ）に出してよいか（2026-09-18 沖）
--
-- きっかけ: 物語ページの写真（看板つきカメラ）に捕獲者名が焼き込まれていた。
-- 今後は捕獲票の入力時に「出してよい／出さない」を選び、出さない人の名前だけ消す。
--
--   individuals.hunter_name_public : true=出してよい / false=出さない / null=未確認（公開ページでは出さない扱い）
--   hunters.name_public            : その人の直近の選択（捕獲票の既定値に使う）
--   story_get / story_get_individual / story_get_event : individual に hunter_public（boolean）を付ける。
--     s.html は hunter_public が true でないとき、写真の看板（左下）を覆って名前を見せない。
--   看板つきカメラ（capture-form.html）は hunter_name_public=false の個体では看板に捕獲者行を入れない。

alter table public.individuals add column if not exists hunter_name_public boolean;
alter table public.hunters add column if not exists name_public boolean;
...
begin
  ...
  select ind.label_id, ind.species, ind.sex, ind.weight_total, ind.capture_date,
         ...
         ind.radiation_test_date, ind.radiation_result, ind.processing_done_at, ind.hunter_name_public into v_ind
  ...
  if v_label is not null then
    select coalesce(jsonb_agg(jsonb_build_object(
             'nickname', v.nickname, 'rating', v.rating, 'dish', v.dish, 'comment', v.comment, 'stamps', v.stamps,
  ...
  v_out := jsonb_build_object(
    ...
    'individual', case when v_ind.label_id is null then null else jsonb_build_object(
      ...
      'hunter_public', coalesce(v_ind.hunter_name_public, false)) end,
...
begin
  ...
  select ind.label_id, ind.species, ind.sex, ind.weight_total, ind.capture_date,
         ind.capture_city, ind.capture_area, ind.capture_method, ind.is_juvenile,
         ind.radiation_test_date, ind.radiation_result, ind.processing_done_at, ind.image_url, ind.hunter_name_public
    into v_ind
  ...
  return jsonb_build_object(
    ...
    'individual', jsonb_build_object(
      ...
      'hunter_public', coalesce(v_ind.hunter_name_public, false)),
    'parts', v_parts,
    'voices', v_voices,
    'voice_count', v_cnt,
    'photo', v_photo,
    -- 出店シートの写真（看板なし）か、看板つきカメラの写真か。看板つきは左下に捕獲者名が焼き込まれている
    'photo_kind', case when v_sheet.photo_path is not null then 'sheet' when v_ind.image_url is not null then 'board' else null end,
...
begin
  ...
  from (
    select distinct on (ind.label_id) jsonb_build_object(
      ...
      'hunter_public', coalesce(ind.hunter_name_public, false),
