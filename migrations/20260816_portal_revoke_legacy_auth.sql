...
-- P0-2 / リンク認証廃止: 旧認証RPC・URLトークン認証RPCの匿名/認証EXECUTEをREVOKE（迂回経路の遮断）。
...
--   ・admin_issue_customer_link(...): 「かんたんログインリンク」用の長期セッション（最大365日）を発行する。
--     このリンクはパスワード・ロックアウトをURLトークンで迂回できてしまうため運用から外す。
--     注文サイトの通常運用（anon/authenticated）から実行不可にする。既存リンクの失効判断は別途（下記）。
-- 適用順の注意: このマイグレーションは**最後**に適用する（クライアント配信後）。
--   旧RPCを使うクライアントが残っている間に先行REVOKEするとログイン不能になるため。
...
-- リンク認証（URLトークンでのログイン迂回）: PUBLIC・匿名・認証から実行不可に。
-- 定義は残すが、注文サイトの通常運用からは呼べなくする（運用一本化＝仮pw方式）。
revoke execute on function public.admin_issue_customer_link(text, uuid[], integer) from public, anon, authenticated;
...
-- 注: 既存の「admin-issued-link」セッション（portal_sessions.user_agent='admin-issued-link'）の失効は、
--     実データへの影響（件数・対象顧客）を確認のうえ別途判断する。本マイグレーションでは削除しない。
--     参考の失効SQL（承認後に実行する場合）:
--       -- delete from public.portal_sessions where user_agent = 'admin-issued-link';
