...
-- ===== 適用: P0-1 RPC（代表 sale_event_settle をガード + トリガrevoke） =====
...
declare
  ...
  v_scan text; prov1 jsonb; prov2 jsonb; k int; rl_blocked boolean := false;
  junk_rejected boolean := false;
begin
  -- P0-6: 平文0件
  select count(*) into pw from customers where portal_password is not null;
  if pw <> 0 then msg := msg || format('portal_password残=%s; ', pw); end if;

  -- P0-2 negative: anon(ヘッダ無し)は staff/hunters が0行、公開VIEWは行あり
  set local role anon;
  ...
  reset role;
  if s <> 0  then msg := msg || format('anon staff可視=%s; ', s); end if;
  if h <> 0  then msg := msg || format('anon hunters可視=%s; ', h); end if;
  if sp = 0  then msg := msg || 'staff_public空; '; end if;
  if hp = 0  then msg := msg || 'hunters_public空; '; end if;

  -- P0-2 #3反映: hunters_public に city / trap_area 列が無い（氏名＋活動地域を出さない）
  if exists (select 1 from information_schema.columns
             where table_name='hunters_public' and column_name in ('city','trap_area')) then
    msg := msg || 'hunters_publicにcity/trap_area残存; ';
  end if;

  -- P0-2 negative: anon は staff/hunters に書けない
  ...
  -- positive: 正当な仮登録は作成され、2回目は冪等(created=false)
  set local role anon;
  ...
  if (prov2->>'created')::boolean is not false then msg := msg || '仮登録2回目created≠false; '; end if;

  -- ★ #2反映(レート制限): 施設全体で1時間20件上限。21件目でブロックされる
  set local role anon;
  begin
    for k in 1..25 loop
      perform public_hunter_provisional('P0RLテスト_' || k::text);
    end loop;
  exception when others then rl_blocked := true;  -- 上限超過で raise される
  end;
  reset role;
