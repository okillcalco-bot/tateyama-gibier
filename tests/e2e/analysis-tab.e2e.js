...
(async () => {
  ...
  ck('解析ページへのリンク（公的データ・捕獲条件・フードマイレージ・売上・過年度・買取・LCA・収支）', ui.links.join() === 'public-analysis.html,catch-analysis.html,mileage-analysis.html,sales-dashboard.html,history.html,buyback.html,lca.html,finance-analysis.html', ui.links.join());
  ...
  ck('準備の状態が3種（取込済／要るもの／稼働中）で出る', /取込済/.test(ui.text) && /が要る/.test(ui.text) && /稼働中/.test(ui.text) && !/今すぐ可/.test(ui.text), '');
