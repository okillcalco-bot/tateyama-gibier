...
//
// 税の二重計上防止（2026-09-08追加）: 価格マスタの単価・order_items.unit_priceはいずれも
// 税込（顧客ポータルのportal_issue_document関数にも明記）だが、invCalcLines()は明細を
// 税抜として税率を上乗せするため、そのまま渡すと肉の分だけ二重課税になっていた。
// 税込金額を税抜相当に変換してから明細に積むよう修正した。
...
(async () => {
  ...
  if (l805) {
    ...
    // 価格マスタの単価3,900円は税込のため、明細には税抜相当（3,900÷1.08≒3,611円）で入る
    // （税込のまま渡すとinvCalcLines()が8%をもう一度乗せる二重課税になるため変換している）
    ck('8/5: 極上の税抜換算単価3,611円が自動で入る（税込3,900円から変換）', Number(l805.price) === 3611, String(l805.price));
  }
  if (l814) {
    ...
    // 同様に税込2,600円→税抜換算2,407円で入る
    ck('8/14: 並の税抜換算単価2,407円が自動で入る（税込2,600円から変換）', Number(l814.price) === 2407, String(l814.price));
  }
